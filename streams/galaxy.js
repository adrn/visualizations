        ...
        if (galaxy_simulation.galaxies.hasOwnProperty(key)) {
            ...
            for (var key in galaxy.populations) {
                if (galaxy.populations.hasOwnProperty(key)) {
                    ...
                    // Get the old positions and velocities
                    var positions = $M(stellar_pop.positions).transpose();
                    var velocities = $M(stellar_pop.velocities).transpose();
                    var num_dimensions = positions.elements[0].length;

                    // Initialize an array of zeros to hold the acceleration at each star position
                    var ai = Matrix.Zero(positions.rows(), num_dimensions);

                    // Compute the total acceleration at a point due to all galaxies
                    for (var key in galaxy_simulation.galaxies) {
                        if (galaxy_simulation.galaxies.hasOwnProperty(key)) {
                            //ai = ai.vectorAdd(galaxy_simulation.galaxies[key].acceleration_at(position));
                            ai.add(galaxy.acceleration_at(positions));
                        }
                    }
                    ...
                    // Compute the new positions
                    var new_positions = $M(positions),
                        new_velocities = $M(velocities);

                    tmp1 = $M(velocities);
                    tmp1.multiply(dt);

                    tmp2 = $M(ai);
                    tmp2.multiply(0.5*dt*dt)

                    tmp1.add(tmp2);

                    new_positions.add(tmp1);
                    ...
                    for (var key in galaxy_simulation.galaxies) {
                        if (galaxy_simulation.galaxies.hasOwnProperty(key)) {
                            new_ai.add(galaxy_simulation.galaxies[key].acceleration_at(new_positions));
                        }
                    }

                    // Compute the new velocities
                    var tmp1 = $M(velocities),
                        tmp2 = $M(ai);

                    tmp2.add(new_ai);
                    tmp2.multiply(0.5*dt);
                    tmp1.add(tmp2);

                    console.log(new_velocities.elements[0]);
                    new_velocities.add(tmp1);
                    console.log(new_velocities.elements[0]);

                    stellar_pop.positions = new_positions.transpose().elements;
                    stellar_pop.velocities = new_velocities.transpose().elements;
                }
            }
        }
    }
...
function LogarithmicGalaxy(position, velocity, mass) {
	...
	_galaxy.acceleration_at = function(xy) {
        ...
        var xx = $V(xy.elements[0]),
            yy = $V(xy.elements[1]);

        xx.subtract(_galaxy.position[0]);
        yy.subtract(_galaxy.position[1]);

        // TODO: Need to update below to use Matrix operations
        tmp1 = $V(xx);
        tmp2 = $V(yy);
        xx.multiply(-1. * _galaxy.mass * 2 * _galaxy.U*_galaxy.U);
        (_galaxy.U*_galaxy.U*xx*xx + yy*yy + _galaxy.C*_galaxy.C*_galaxy.U*_galaxy.U)

        var xdotdot = - _galaxy.mass * 2 * _galaxy.U*_galaxy.U * xx / (_galaxy.U*_galaxy.U*xx*xx + yy*yy + _galaxy.C*_galaxy.C*_galaxy.U*_galaxy.U);
        var ydotdot = - _galaxy.mass * 2 * yy / (_galaxy.U*_galaxy.U*xx*xx + yy*yy + _galaxy.C*_galaxy.C*_galaxy.U*_galaxy.U);

        return $M([xdotdot, ydotdot]);
    }
